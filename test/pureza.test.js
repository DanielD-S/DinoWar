...
import { tablero, enMano } from './helpers.js';
...
test('reduce() no muta el estado que recibe', () => {
  ...
  reduce(s, { tipo: ACCION.DESPLEGAR, jugador: 0, iid: s.jugadores[0].mano[0], ranura: 0 });
  reduce(s, { tipo: ACCION.DESPLEGAR, jugador: 0, iid: -1, ranura: 9 });   // ilegal
...
test('Una acción ilegal no lanza y deja el estado funcionalmente intacto', () => {
  ...
  const r = reduce(s, { tipo: ACCION.DESPLEGAR, jugador: 0, iid: 99999, ranura: 2 });
...
test('vistaDe() oculta la mano y el despliegue del rival', () => {
  ...
  s0.jugadores[1].biomasa = 8;
  enMano(s0, 'allosaurus', 1);
  const oculta = enMano(s0, 'stegosaurus', 1);
  const s = reduce(s0, { tipo: ACCION.DESPLEGAR, jugador: 1, iid: oculta, ranura: 1 });
