...
// contador que no existe nunca avanza; una recompensa que apunta a un título
// que no está en el catálogo se «entrega» y no da nada; y el servidor tiene
// que apuntar el progreso por los tres caminos —partida, asalto y duelo—.
...
test('Toda recompensa es entregable: un título del catálogo, un mazo o sobres', () => {
  for (const l of LOGROS) {
    const r = l.recompensa;
    assert.ok(Object.values(RECOMPENSA).includes(r.tipo), `«${l.id}» da «${r.tipo}»`);
    if (r.tipo === RECOMPENSA.TITULO) {
      const c = cosmeticoPorId(r.id);
      assert.ok(c?.tipo === 'TITULO' && c.exclusivo && c.logro === l.id,
        `«${l.id}» da «${r.id}», que no es un título exclusivo suyo`);
    }
  ...
  // Y al revés: todo título exclusivo lo da algún logro.
  for (const c of COSMETICOS.filter((x) => x.tipo === 'TITULO' && x.exclusivo)) {
...
test('Los contadores nuevos están en el parte vacío y sólo avanza lo que se toca', () => {
  ...
  for (const k of ['asaltos', 'danoJefe', 'jefesVencidos', 'duelos', 'duelosGanados', 'expedicionNuevos']) {
    assert.ok(VOCABULARIO.includes(k));
    assert.equal(p[k], 0);
  }
  assert.deepEqual(avancesDeLogros(p), []);
  const a = avancesDeLogros({ ...p, duelos: 1, duelosGanados: 1 });
  assert.deepEqual(a.map((x) => x.id).sort(), ['campeon', 'duelista', 'veterano']);
...
test('El servidor apunta el progreso y entrega la recompensa que viaja en la llamada', () => {
  ...
  // Las tres recompensas, entregadas en SQL.
  assert.match(SQL, /when 'titulo' then perform private\.otorgar_cosmetico/);
  assert.match(SQL, /when 'mazo' then update public\.jugadores set mazos_extra = mazos_extra \+ 1/);
  assert.match(SQL, /when 'sobres' then update public\.jugadores set sobres_gratis = sobres_gratis \+/);
