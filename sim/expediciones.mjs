...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  for (const e of EXPEDICIONES) {
    ...
    process.stdout.write(`  ${'#'.padEnd(3)}${'rival'.padEnd(30)}${'perfil'.padEnd(12)}le ganas  turnos  vías\n`);
    e.rivales.forEach((r, i) => {
      const m = medir(r, n);
      const vias = Object.entries(m.vias).map(([k, v]) => `${k[0]}${Math.round((v / n) * 100)}`).join(' ');
      process.stdout.write(`  ${String(i + 1).padEnd(3)}${r.nombre.padEnd(30)}${r.perfil.padEnd(12)}${pct(m.gana)}  ${m.turnos.toFixed(1).padStart(6)}  ${vias}\n`);
    });
  }
  process.stdout.write('\nVisitantes de la semana\n');
  for (const r of VISITANTES) {
    const m = medir(r, n);
    process.stdout.write(`     ${r.nombre.padEnd(30)}${r.perfil.padEnd(12)}${pct(m.gana)}  ${m.turnos.toFixed(1).padStart(6)}\n`);
  }
}
